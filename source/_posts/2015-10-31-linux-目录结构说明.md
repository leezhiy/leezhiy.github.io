...
![文件系统](/images/linux-filesystem.png)
...
### 环境变量配置文件
        
文件目录：`/etc/profile` ，用来存放用户的环境变量配置（例如umask等）在登陆shell设置环境变量的时候，该文件会调用 `/etc/profile.d` 目录下的文件，我们定义别名，或者其他变量的时候，可以在 `/etc/profile.d` 下创建文件即可。
...
注意：使用 `free –m`（设置单位为Mb）也可以查看内存的使用情况
